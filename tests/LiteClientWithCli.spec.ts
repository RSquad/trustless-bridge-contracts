...
console.log(newKeyBlockSignatures);
...
describe("LiteClientCLI", () => {
  ...
  beforeEach(async () => {
    ...
    liteClient = blockchain.openContract(
      LiteClient.createFromConfig(
        { validators: initEpoch.shortValidators, totalWeight: initEpoch.totalWeight, validatorsHash: initEpoch.validatorsHash },
        code,
      ),
    );
  ...
  it("should check block", async () => {
    ...
    // console.log(result.transactions.map(t => t.vmLogs));
